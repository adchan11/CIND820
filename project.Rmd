...
df_corr_binary <- df_corr_binary %>%
  select(-IBS_NASL)

#Split into two datasets so that the correlation plot is easier to read 

df_corr_binary_1 <- df_corr_binary[c(1:24, 52)]
df_corr_binary_2 <- df_corr_binary[25:52]
#Generate significance values at confidence level = 0.95
testRes_1 <- cor.mtest(df_corr_binary_1, conf.level = 0.95)
testRes_2 <- cor.mtest(df_corr_binary_2, conf.level = 0.95)

#Generate correlation plot with statistical significance
corrplot(cor(df_corr_binary_1, use = "pairwise.complete.obs"), p.mat = testRes_1$p, method = "color", diag = FALSE, type = 'upper',
         sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.8, 
         insig = 'label_sig', pch.col = 'purple', na.label = "NA", number.cex = 0.5, tl.cex = 0.8)

corrplot(cor(df_corr_binary_2, use = "pairwise.complete.obs"), p.mat = testRes_2$p, method = "color", diag = FALSE, type = 'upper',
         sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.8, 
         insig = 'label_sig', pch.col = 'purple', na.label = "NA", number.cex = 0.5, tl.cex = 0.8)
...
chisqmatrix <- function(x) {
  ...
  for (i in 1:(num-1)) {
    for (j in (i+1):num) {
      m[i,j] <- chisq.test(x[,i],x[,j],simulate.p.value = TRUE)$p.value
    }
  }
  return (m)
}
