...
******
\newpage
## Predictive Modeling - without hyperparameter tuning and SMOTE
```{r}
#Logistic Regression
set.seed(1)
...
set.seed(1)
system.time(RF_model <- randomForest(any_complication ~., data = train, proximity = TRUE, na.action = na.exclude))
...
library(nnet)
set.seed(1)
system.time(NN_model <- nnet::nnet(any_complication ~ ., data = train, size = 10))
...
******
\newpage
## Predictive Modeling - with hyperparameter tuning and SMOTE
```{r}
#Logistic Regression
set.seed(1)

#Perform Stepwise Logistic Regression
step_LR_model_tuned <- train(any_complication ~.,
                     df,
...
summary(step_LR_model_tuned)

#Reference
# https://stackoverflow.com/questions/70938683/caret-cross-validation-following-stepwise-selection-question-on-mechanism

#Predict results using test dataset
# preds <- predict(step_LR_model_tuned)

#Confusion matrix and statistics
caret::confusionMatrix(step_LR_model_tuned$pred$pred, step_LR_model_tuned$pred$obs, mode = "everything", positive = '1')

#Reference
# https://community.rstudio.com/t/compute-confusion-matrix-using-k-fold-cross-validation-in-caret-train/42412/3

#Create ROC and AUC
library(pROC)
roc <- roc(response = as.numeric(step_LR_model_tuned$pred$obs), predictor = as.numeric(step_LR_model_tuned$pred$pred), smoothed = TRUE,
            # arguments for ci
            ci=TRUE, ci.alpha=0.95, stratified=FALSE,
            # arguments for plot
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
...
#Train model
set.seed(1)
system.time(NB_model_tuned <- train(any_complication ~.,
                     df,
                     method="nb", 
                     trControl=train_ctrl))
...
#Confusion matrix and statistics
caret::confusionMatrix(NB_model_tuned$pred$pred, NB_model_tuned$pred$obs, mode = "everything", positive = '1')

#Create ROC and AUC
roc <- roc(response = as.numeric(NB_model_tuned$pred$obs), predictor = as.numeric(NB_model_tuned$pred$pred),  smoothed = TRUE,
            # arguments for ci
            ci=TRUE, ci.alpha=0.95, stratified=FALSE,
            # arguments for plot
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
