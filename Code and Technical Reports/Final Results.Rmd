...
#Train model
set.seed(1)
system.time(NB_model <- naiveBayes(as.factor(any_complication) ~ ., data = train))
...
set.seed(1)
system.time(RF_model <- randomForest(any_complication ~., data = train, proximity = TRUE,
                                     ntree = 500, na.action = na.exclude))
...
library(randomForest)
set.seed(1)
system.time(RF_model <- randomForest(any_complication ~., data = train_smote, proximity = TRUE,
                                     ntree = 500, na.action = na.exclude))
...
library(nnet)
set.seed(1)
system.time(NN_model <- nnet::nnet(any_complication ~ ., data = train_smote, size = 1))
...
library(MASS)
system.time(step_LR_model <- full_LR_model_smote %>% stepAIC(trace = FALSE))
...
#Train model
set.seed(1)
system.time(NB_model_tuned <- naiveBayes(as.factor(any_complication) ~ ., data = train_smote, laplace = 1))
...
#Hypertuning - find best mtry value
set.seed(1)

bestmtry <- tuneRF(train_smote[,1:18], train_smote[, 19], stepFactor = 1.5, improve = 1e-5, ntreeTry = 500)
#According to the tuning, mtry = 3 has the lowest error rate
#https://rpubs.com/phamdinhkhanh/389752

system.time(RF_model_tuned <- randomForest(any_complication ~., data = train_smote, proximity = TRUE, 
                                           mtry = 3, ntree = 500, na.action = na.exclude))

#Predict results using test dataset
preds <- predict(RF_model_tuned, test)
...
library(neuralnet)
set.seed(1)
system.time(bestparam <- train(any_complication~., data=train_smote, method='nnet', trace=F,
...
# system.time(NN_model_tuned <- nnet::nnet(any_complication ~ ., data = train_smote, size = 4, decay = 0.1))

#Predict results using test dataset
result <- predict(bestparam, test, type = 'raw')
preds <- ifelse(result > 0.5, 1,0 )

# presult <- predict(NN_model_tuned, test)
# preds <- ifelse(result > 0.5, 1,0 )
