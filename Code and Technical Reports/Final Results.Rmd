...
# train_ctrl <- trainControl(method = "cv", number = 10, sampling = "smote", savePredictions=TRUE)
# train_ctrl_no_smote <- trainControl(method = "cv", number = 10, savePredictions=TRUE)
...
# https://stackoverflow.com/questions/60020654/when-using-the-train-function-with-cross-validation-train-control-how-do-you
```


******
\newpage
## Predictive Modeling - Before SMOTE
...
#Train model
system.time(full_LR_model <- glm(any_complication ~.,family = binomial(link = 'logit'), data = train, na.action = na.omit))
...
result <- predict(full_LR_model, newdata = test, type = 'response')
...
library(naivebayes)
system.time(NB_model <- naive_bayes(any_complication ~., data = train))
...
system.time(RF_model <- ranger(
    formula         = any_complication ~ ., 
    data            = train
  ))

#Predict results using test dataset
preds <- predict(RF_model, test)$predictions
...
#Train model
set.seed(1)
# system.time(NN_model <- train(any_complication~., data=train, method='nnet', trace=F))

system.time(NN_model <- nnet::nnet(any_complication~., data=train, size = 1))
...
******
\newpage
## Predictive Modeling - After SMOTE
...
#Train model
system.time(full_LR_model_smote <- glm(any_complication ~.,family = binomial(link = 'logit'), data = train_smote, na.action = na.omit))
summary(full_LR_model_smote)

#https://www.datacamp.com/community/tutorials/logistic-regression-R
#https://towardsdatascience.com/machine-learning-with-r-logistic-regression-152ec20351db

#Predict results using test dataset
result <- predict(full_LR_model_smote, newdata = test, type = 'response')
...
#Train model
set.seed(1)
system.time(NB_model_smote <- naive_bayes(any_complication ~., data = train_smote))

#Predict results using test dataset
preds <- predict(NB_model_smote, test)
...
#Train model
set.seed(1)
system.time(RF_model_smote <- ranger(
    formula         = any_complication ~ ., 
    data            = train_smote
  ))

#Predict results using test dataset
preds <- predict(RF_model_smote, test)$predictions
...
#Train model
set.seed(1)
system.time(NN_model_smote <- nnet::nnet(any_complication~., data=train_smote, size = 1))
            
#Predict results using test dataset
result <- predict(NN_model_smote, newdata = test)
preds <- ifelse(result > 0.5, 1, 0)
...
library(MASS)
system.time(step_LR_model <- full_LR_model %>% stepAIC(direction = 'both', trace = FALSE))
...
# https://stackoverflow.com/questions/47820750/training-model-in-caret-using-f1-metric
...
# train model
system.time(NB_model_tuned <- train(
  any_complication ~.,
  train,
  method = "naive_bayes",
...
# # top 5 models
# NB_model_tuned$results %>% 
#   top_n(5, wt = Accuracy) %>%
#   arrange(desc(Accuracy))

# https://uc-r.github.io/naive_bayes#caret

system.time(NB_model_tuned <- naive_bayes(any_complication ~., data = train, laplace = 0, 
                                          usekernel = TRUE, adjust = 1))

#Predict results using test dataset
preds <- predict(NB_model_tuned, test)
...
for(i in 1:nrow(hyper_grid)) {
  ...
  hyper_grid$F1[i] <- MLmetrics::F1_Score(preds,test$any_complication, positive = 1)
}
...
system.time(RF_model_tuned <- ranger(
    formula         = any_complication ~ ., 
    data            = train, 
    num.trees       = 500,
    mtry            = 2,
...
#Predict results using test dataset
preds <- predict(RF_model_tuned, test)$predictions
...
# https://www.r-bloggers.com/2021/04/random-forest-in-r/
# https://discuss.analyticsvidhya.com/t/what-does-the-warning-the-response-has-five-or-fewer-unique-values-while-building-random-forest-mean/6442/2
# https://medium.com/@ammubharatram/hyperparameter-tuning-of-random-forest-in-both-r-and-python-747b06b568db
...
bestparam <- train(any_complication~., data=train, method='nnet', trace=F, 
                   trControl = train_control,tuneGrid=expand.grid(.size=c(1,2,3,4),                                   .decay=c(0.7,0.8,0.9, 1)), metric = "F1")
...
# 
# bestparam <- train(any_complication~., data=train, method='nnet', trace=F,
#                    trControl = train_control, tuneLength = 40, metric = "F1")

bestparam

# https://www.mydatahack.com/predict-internet-popularity-by-optimising-neural-networks-with-r/
# https://stackoverflow.com/questions/42417948/how-to-use-size-and-decay-in-nnet

system.time(NN_model_tuned <- nnet::nnet(any_complication~., data=train, size = 1, decay = 0.8))
            
#Predict results using test dataset
result <- predict(NN_model_tuned, newdata = test)
preds <- ifelse(result > 0.5, 1, 0)
